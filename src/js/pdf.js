...
function generarPDF(boton) {
  ...
  doc.text("Telecomunicaciones", 8, 10);
  doc.text("Detalle de la Venta", 8, 20);
...
function generarPDFPG(boton) {
  ...
  // Obtener datos del formulario
  const nombreCliente = fila.querySelector(".clientepago").textContent;
  const nombreVendedor = fila.querySelector(".cajero").textContent;
  const producto = fila.querySelector(".servicio").textContent;
  const fechaVenta = fila.querySelector(".fechapago").textContent;
  const precio = fila.querySelector(".pago").textContent;
  const mensaje = 'gracias por su compra';
  ...
  doc.text("Telecomunicaciones", 8, 10);
  doc.text("Detalle del pago", 8, 20);

  doc.text(`Nombre del cliente: ${nombreCliente}`, 8, 30);

  doc.text(`Fecha de pago: ${fechaVenta}`, 8, 40);
  doc.text(`Servicio contratado: ${producto}`, 8, 50);
  doc.text(`Monto pagado: $${precio}`, 8, 70);
